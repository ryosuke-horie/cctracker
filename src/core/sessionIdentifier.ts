import { addHours, differenceInMinutes, isAfter, startOfHour } from 'date-fns';
...
export class SessionIdentifier {
  ...
  private alignToResetHour(timestamp: Date): Date {
    ...
    const alignedHour =
      this.resetHours.filter((h) => h <= hour).pop() || this.resetHours[this.resetHours.length - 1];
  ...
  private finalizeBlock(block: SessionBlock): void {
    if (block.actualEndTime) {
      block.durationMinutes = differenceInMinutes(block.actualEndTime, block.startTime);
    } else {
      block.durationMinutes = differenceInMinutes(new Date(), block.startTime);
    }
  }
